...
export default function EditProfilePage() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      if (currentUser) {
        ...
        router.push('/auth/login?redirect=/profile/edit'); // Redirect if not logged in
      }
      setLoading(false);
    });
    return () => unsubscribe();
  ...
  if (!user) {
    // This case is primarily for when the redirect hasn't completed yet or as a fallback.
    return (
      <div className="container mx-auto flex min-h-[calc(100vh-10rem)] flex-col items-center justify-center py-12 px-4 text-center">
        <Card className="w-full max-w-md shadow-lg">
          ...
          <CardContent>
            <p className="text-muted-foreground mb-6">You must be logged in to edit your profile.</p>
            <Button asChild size="lg">
              <Link href="/auth/login?redirect=/profile/edit">Login to Continue</Link>
            </Button>
          </CardContent>
        </Card>
      </div>
  ...
  return (
    <div className="container mx-auto flex min-h-[calc(100vh-10rem)] items-center justify-center py-12 px-4">
      <UserProfileEditForm userId={user.uid} />
    </div>
  );
}
