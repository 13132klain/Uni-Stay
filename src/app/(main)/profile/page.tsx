...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      if (currentUser) {
        ...
        router.push('/auth/login?redirect=/profile'); // Redirect if not logged in
      }
      setLoading(false);
    });
    return () => unsubscribe();
            ...
            <Card className="w-full max-w-md shadow-lg">
                ...
                <CardContent>
                    ...
                    <Button asChild size="lg">
                        <Link href="/auth/login?redirect=/profile">Login to Continue</Link>
                    </Button>
                </CardContent>
            </Card>
        </div>
  ...
  return (
    <div className="container mx-auto py-12 px-4">
      <UserProfileDisplay userId={user.uid} />
    </div>
  );
}
